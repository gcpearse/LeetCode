...
import { buildLines, fullJustify } from "../hard/0068-text-justification"
...
describe("fullJustify", () => {
  ...
  describe("fullJustify", () => {
    test("Returns an array of fully justified text", () => {
      ...
      expect(fullJustify(["What", "must", "be", "acknowledgement", "shall", "be"], 16)).toEqual([
        "What   must   be",
        "acknowledgement ",
        "shall be        "
      ])
  ...
  describe("buildLines", () => {
    test("Returns an array of arrays each of which contains unpadded words respecting the character limit for a line", () => {
      expect(buildLines(["This", "is", "an", "example", "of", "text", "justification."], 16)).toEqual([
        ["This", "is", "an"],
        ["example", "of", "text"],
        ["justification."]
      ])
      expect(buildLines(["What", "must", "be", "acknowledgement", "shall", "be"], 16)).toEqual([
        ["What", "must", "be"],
        ["acknowledgement"],
        ["shall", "be"]
      ])
